...
class MS4(override val request: Request) extends PIIParser {
    ...
    if (body != null && body.nonEmpty) {
      try {
        if (body.startsWith("1:")) {
          ...
          warn(s"we do not support obfuscation :2")
      ...
      warn(s"we are missing ${binary.length} obfuscated binary data")
      JsObject(
  ...
  override protected def parseQuery(implicit queryValues: JsObject,
    ...
    addPii(PiiTypes.DEOBFUSCATED, "deobfuscated") // this sadly never happens for iOS
  }

  override protected def parseBody(implicit bodyValues: JsObject,
                                   PIILocation: PIILocation): Unit = {
    addPii(PiiTypes.ID_UUID, "alt")
